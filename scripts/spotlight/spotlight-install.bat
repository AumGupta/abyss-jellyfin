...
powershell -NoProfile -Command "$p=$env:ABYSS_INDEX; $h=[IO.File]::ReadAllText($p); $h=[regex]::Replace($h,'<script[^>]*\bdata-abyss-spotlight\b[^>]*></script>','',[Text.RegularExpressions.RegexOptions]::IgnoreCase); if ($h -notmatch '(?i)</body>') { throw 'index.html has no closing body tag' }; $u=Join-Path (Split-Path $p) 'ui'; $f=@('spotlight-loader.js','spotlight.html','spotlight.css')|%%{Join-Path $u $_}; foreach($x in $f){if(-not(Test-Path $x)){throw 'Spotlight asset not found: '+$x}}; $m=New-Object IO.MemoryStream; $a=[Security.Cryptography.SHA256]::Create(); try { foreach($x in $f){$s=[IO.File]::OpenRead($x);try{$s.CopyTo($m)}finally{$s.Dispose()}}; $v=([BitConverter]::ToString($a.ComputeHash($m.ToArray()))).Replace('-','').ToLowerInvariant().Substring(0,12) } finally { $a.Dispose(); $m.Dispose() }; $t='<script src='+[char]34+'ui/spotlight-loader.js?v='+$v+[char]34+' data-abyss-spotlight></script>'; $r=[regex]::new('</body>',[Text.RegularExpressions.RegexOptions]::IgnoreCase); $h=$r.Replace($h,$t+'</body>',1); $tmp=Join-Path (Split-Path $p) ('.abyss-index-'+[guid]::NewGuid().ToString('N')); try { [IO.File]::WriteAllText($tmp,$h,(New-Object Text.UTF8Encoding($false))); [IO.File]::Replace($tmp,$p,$null) } finally { if (Test-Path $tmp) { Remove-Item $tmp -Force } }"
if errorlevel 1 (
